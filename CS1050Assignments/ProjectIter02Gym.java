...
public class ProjectIter02Gym {
	public static void main(String[] args) {
		...
		try {
			...
			/**
			 * Reads athletes file Display athlete's info Save athlete's info to a different
			 * file
			 */
	...
	public static void readGymAthletesInfoFromFile(String filename, Gym currentGym) throws FileNotFoundException {
		...
		try {
			...
			while (fileScanner.hasNextLine()) {
				...
				/**
				 * Create new athlete Add athlete to gym file
				 */
...
//Adds athlete to the gym
	...
	/**
	 * Displays summary for each athlete's data Identify who is top athlete Identify
	 * which athlete is underweight
	 */
	public void displayAthleteSummaries() {
		for (int i = 0; i < numAthletes; i++) {
			...
			System.out.println(
					"        Average Daily Calories Burned: " + Math.round(athlete.getAverageCaloriesBurned()));
	...
	public void saveReportToFile() {
		...
		try (PrintWriter writer = new PrintWriter(fileName)) {
			...
			for (int i = 0; i < numAthletes; i++) {
				...
				writer.println(
						"        Average Daily Calories Burned: " + Math.round(thisAthlete.getAverageCaloriesBurned()));
				writer.println("        BMI: " + thisAthlete.calculateBMI() + "     Category: "
						+ thisAthlete.getBMICategory());
			}
...
//Identify athlete with the highest average calories burn
	private void findTopAthlete() {
		...
		System.out.println(
				"Top Athlete: " + athletes[topAthlete].getFirstName() + " " + athletes[topAthlete].getLastName());
	}// End findTopAthlete
...
//Identify the lowest calories burn athlete
	private void displayUnderweightAthletes() {
		...
		System.out.println("Underweight Athlete: " + athletes[underweightAthlete].getFirstName() + " "
				+ athletes[underweightAthlete].getLastName());
